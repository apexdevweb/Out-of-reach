<section class="tips__container">
  <div class="tips__sub--container">
    <a href="#" class="tips__el--link"><?= DataText::SECTOR_ENGINERING ?><span class="minusOrPlus">+</span></a>
    <article class="tips__el">
      <header class="tips__el--head">
        <hgroup class="tips__el--title-container">
          <h4 class="tips__title"><?= DataText::WEB_ENGINEERING_TITLE ?></h4>
        </hgroup>
      </header>
      <p class="tips__content"><?= DataText::ARCHITECTUR_DESCRIPTION ?></p>
      <figure class="tips__img--container">
        <div class="tips__link--container">
          <strong class="tips__link--title">Javascript</strong>
          <a href="index.php?page=<?= Encryptor::encrypt('js_structure') ?>" class="tips__link">>_</a>
        </div>
        <div class="tips__link--container">
        <strong class="tips__link--title">Php</strong>
          <a href="index.php?page=<?= Encryptor::encrypt('php_structure') ?>" class="tips__link">>_</a>
        </div>
        <div class="tips__link--container">
        <strong class="tips__link--title">Python</strong>
          <a href="index.php?page=<?= Encryptor::encrypt('py_structure') ?>" class="tips__link">>_</a>
        </div>
        <div class="tips__link--container">
        <strong class="tips__link--title">Rust</strong>
          <a href="index.php?page=<?= Encryptor::encrypt('rust_structure') ?>" class="tips__link">>_</a>
        </div>
        <figcaption class="tips__img--caption"></figcaption>
      </figure>
      <footer class="tips__el--foot">
        <p class="tips__foot--txt"><?= DataText::TIPS_FOOTER_TXT ?></p>
      </footer>
    </article>
  </div>
  <div class="tips__sub--container">
    <a href="#" class="tips__el--link"><?= DataText::SECTOR_DEFENCE ?><span class="minusOrPlus">+</span></a>
    <article class="tips__el">
      <header class="tips__el--head">
        <hgroup class="tips__el--title-container">
          <h4 class="tips__title"></h4>
          <h5 class="tips__title"></h5>
        </hgroup>
      </header>
      <p class="tips__content"></p>
      <figure class="tips__img--container">
        <figcaption class="tips__img--caption"></figcaption>
      </figure>
      <footer class="tips__el--foot"></footer>
    </article>
  </div>
  <div class="tips__sub--container">
    <a href="#" class="tips__el--link"><?= DataText::SECTOR_ATTACK ?><span class="minusOrPlus">+</span></a>
    <article class="tips__el">
      <header class="tips__el--head">
        <hgroup class="tips__el--title-container">
          <h4 class="tips__title"></h4>
          <h5 class="tips__title"></h5>
        </hgroup>
      </header>
      <p class="tips__content"></p>
      <figure class="tips__img--container">
        <figcaption class="tips__img--caption"></figcaption>
      </figure>
      <footer class="tips__el--foot"></footer>
    </article>
  </div>
</section>